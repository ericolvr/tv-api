...
PROCESSED_FILE = (
    Path(__file__).parents[2] / "data" / "processed" / "processed_data.parquet"
)
...
def get_by_program(program_code: str, exhibition_date: date) -> list[dict]:
    ...
    mask = (df["program_code"] == program_code) & (
    ...
    if result["predicted_audience"].isna().any():
        ...
        if fallback_mask.any():
            ...
            result["predicted_audience"] = result["predicted_audience"].fillna(
                median_val
            )
